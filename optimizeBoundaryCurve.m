...
while area_overlap > overlap_thr
    ...
    iterFile = strcat(folder, 'overlap_iter', num2str(iter), '.png');
...
iter = 0;
gap_thr = 0.001;
disp('===Start eleminating gaps===');
prev_gap = area_gap;
while area_gap > gap_thr
    if iter >= max_iter
        break;
    end
    [curves, area_gap, area_overlap] = deformCurve_lap_gap(curves, T_P, feaIds);
    deformEnergy = 0; % getDeformationEnergy(curves, prev);
    iter = iter + 1;    
    ...
    tran_curve = transform_curves(curves, T_Q, scale);
    show_curves(curves, tran_curve, 1);
    iterFile = strcat(folder, 'gap_iter', num2str(iter), '.png');
    str = strcat('Iter ',  num2str(iter), ...
        ': overlap-area: ', num2str(area_overlap), ...
        ', gap-area: ', num2str(area_gap));
    text(-1, 1, str);
